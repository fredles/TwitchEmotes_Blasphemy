--[[
    TwitchEmotes_Blasphemy Animation System
    ...
    2. PLACE THE FILE
       Copy the .tga file to: TwitchEmotes_Blasphemy/emotes/
    
    3. REGISTER THE EMOTE (in Emotes.lua)
       
       a) Add to TwitchEmotes_Blasphemy_Emoticons:
          ["mbroMeow"] = "mbroMeow",
       
       b) Add to TwitchEmotes_Blasphemy_Emoticons_Pack (NO :28:28 suffix for animated!):
          ["mbroMeow"] = "Interface\\AddOns\\TwitchEmotes_Blasphemy\\emotes\\mbroMeow.tga",
       
        c) Add to TwitchEmotes_Blasphemy_Animation_Metadata:
           ["Interface\\AddOns\\TwitchEmotes_Blasphemy\\emotes\\mbroMeow.tga"] = {
    ...
    Example for a 3:1 aspect ratio emote (96x32 frames):
       ["Interface\\AddOns\\TwitchEmotes_Blasphemy\\emotes\\wideEmote.tga"] = {
           nFrames = 36,
...
local Blasphemy_T = 0
local Blasphemy_TimeSinceLastUpdate = 0
local Blasphemy_UPDATE_INTERVAL = 0.033 -- ~30 FPS
...
-- Pattern to match Blasphemy emote textures in chat
local Blasphemy_TEXTURE_PATTERN = "(|TInterface\\AddOns\\TwitchEmotes_Blasphemy\\emotes\\.-|t)"
local Blasphemy_PATH_PATTERN = "|T(Interface\\AddOns\\TwitchEmotes_Blasphemy\\emotes\\.-%.tga).-|t"
...
function TwitchEmotes_Blasphemy_GetCurrentFrameNum(animdata)
    if animdata.pingpong then
        -- Pingpong mode: play forward then backward
        local totalFrames = (animdata.nFrames * 2) - 1
        local virtualFrame = math.floor((Blasphemy_T * animdata.framerate) % totalFrames)
    ...
    return math.floor((Blasphemy_T * animdata.framerate) % animdata.nFrames)
end
...
function TwitchEmotes_Blasphemy_GetTexCoordsForFrame(animdata, framenum)
...
function TwitchEmotes_Blasphemy_BuildEmoteFrameString(imagepath, animdata, framenum, width, height)
...
--[[
    ...
    Finds all Blasphemy emote textures and updates their texture coordinates
...
function TwitchEmotes_Blasphemy_UpdateEmoteInFontString(fontstring, widthOverride, heightOverride)
    ...
    -- Find all Blasphemy emote textures in the text
    for emoteTextureString in txt:gmatch(Blasphemy_TEXTURE_PATTERN) do
        -- Extract the image path from the texture string
        local imagepath = emoteTextureString:match(Blasphemy_PATH_PATTERN)
        
        if imagepath then
            -- Check if this emote has animation metadata
            local animdata = TwitchEmotes_Blasphemy_Animation_Metadata and 
                           TwitchEmotes_Blasphemy_Animation_Metadata[imagepath]
            ...
            if animdata then
                ...
                local framenum = TwitchEmotes_Blasphemy_GetCurrentFrameNum(animdata)
                local newTextureString = TwitchEmotes_Blasphemy_BuildEmoteFrameString(
...
function TwitchEmotes_Blasphemy_Animator_OnUpdate(self, elapsed)
    -- Accumulate time
    Blasphemy_T = Blasphemy_T + elapsed
    Blasphemy_TimeSinceLastUpdate = Blasphemy_TimeSinceLastUpdate + elapsed
    
    -- Only update visuals at target FPS to save performance
    if Blasphemy_TimeSinceLastUpdate < Blasphemy_UPDATE_INTERVAL then
        return
    end
    Blasphemy_TimeSinceLastUpdate = 0
        ...
        for _, frameName in pairs(CHAT_FRAMES) do
            ...
            if frame and frame:IsShown() and frame.visibleLines then
                for _, visibleLine in ipairs(frame.visibleLines) do
                    ...
                    TwitchEmotes_Blasphemy_UpdateEmoteInFontString(visibleLine, nil, nil)
                end
            end
        end
    end
       ...
       EditBoxAutoCompleteBox.existingButtonCount then
        for i = 1, EditBoxAutoCompleteBox.existingButtonCount do
            ...
            if btn and btn:IsVisible() then
                TwitchEmotes_Blasphemy_UpdateEmoteInFontString(btn, nil, 16)
            else
                break
            end
        end
...
function TwitchEmotes_Blasphemy_InitAnimator()
    -- Create a frame to receive OnUpdate events
    local animatorFrame = CreateFrame("Frame", "TwitchEmotes_Blasphemy_AnimatorFrame", UIParent)
    animatorFrame:SetScript("OnUpdate", TwitchEmotes_Blasphemy_Animator_OnUpdate)
end
